...
| `WorldgenPipeline.java` | WORLDGEN injection (restart-gated): generates a block + `WorldgenAgent` ore params, then writes `configured_feature` / `placed_feature` / `neoforge:biome_modifier` JSON into the datapack and stores a `worldgenIntent` on the slot. Worldgen registries freeze at world load, so the feedback tells the player to **rejoin** to see it spawn. Reached via `/conjure new` for ore-like prompts. |
