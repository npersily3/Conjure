...
| `WorldgenPipeline.java` | WORLDGEN injection (restart-gated): generates a plain ore block + `WorldgenAgent` params, then writes `configured_feature` / `placed_feature` / `neoforge:biome_modifier` JSON (named by block slot) into the datapack and stores a `worldgenIntent` on the slot. Worldgen registries freeze at world load, so the feedback tells the player to **rejoin** to see it spawn. Reached via `/conjure new` for ore-like prompts. |
