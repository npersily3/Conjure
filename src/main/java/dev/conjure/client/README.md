...
| `ConjureHud.java` | "Conjure is thinking" indicator driven by `GenerationStatus`. Inner `Registrar` registers a HUD layer (`RegisterGuiLayersEvent`) that draws a pulsing amber square + animated "Conjuring…" in the top-left while generation runs; inner `FocusGuard` (client tick) disables `pauseOnLostFocus` while busy so you can alt-tab and generation keeps running, restoring the setting when idle. |
