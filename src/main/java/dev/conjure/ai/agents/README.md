...
| `ModPlannerAgent.java` | Decomposes a request into pieces. `plan` (flat, for `/conjure new`): EXPANSIVE or AUTO list of prompts. `planMod` (for `/conjure mod`): a structured **gameplay-loop economy** — an ordered `ModPlan` of `Piece`s with roles (resource→material→product) and crafting dependencies (`from`), so the mod has a real progression. Falls back to flat if the structured reply is unparseable. |
| `ScriptReviewer.java` | Static pre-release review of a generated behavior script: flags `ctx.*` calls that aren't real methods and string-literal effect/particle/sound/item/block/entity ids that aren't in the frozen `BuiltInRegistries`. Findings drive `LogicAgent`'s targeted repair. Catches the runtime errors the compile-check can't. |
