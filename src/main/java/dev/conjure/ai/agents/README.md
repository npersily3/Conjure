...
| `DataAgent.java` | Generates a display name + one-sentence description, a split `visualIntent` / `usageIntent` (what it should look like vs. do — for the debug tooltip overlay, falling back to the prompt if the model omits them), and classifies whether a block is a building `isMaterial` (with which shaped `variants` apply). Returns a `Result` record. |
