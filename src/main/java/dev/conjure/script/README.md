...
| `ScriptContext.java` | The `ctx` host object injected into every script. Provides the whitelisted gameplay API: `message(text)`, `giveItem(itemId, count)`, `heal(amount)`, `damage(amount)`, `spawnParticleHere()`, `getPlayerName()`. No Java imports, no IO, no reflection. |
