...
| `ScriptErrorLog.java` | Bounded in-memory log of script runtime failures (scriptId → latest error), pushed from the item/block catch blocks. Drained by `/conjure fixscripts`, which feeds each to the fixer agent and hot-reloads the corrected script — the live half of the review channel. |
