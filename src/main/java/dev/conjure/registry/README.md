...
| `ConjureWorldgen.java` | Pre-registers a pool of worldgen tracking slots for runtime ore injection. `WorldgenPipeline` writes the actual `configured_feature`/`placed_feature`/`biome_modifier` JSON as datapack files (which load at world join), so this is a slot-tracking pool, not live registry objects. |
